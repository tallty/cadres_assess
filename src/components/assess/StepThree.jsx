...
export class StepThree extends Component {
  ...
  get_self_list() {
    ...
    for (let i = 0; i < 12; i++) {
      let item = this.state.self_evaluation.self_evaluation_duties[i];
      item = item ? item : ['', ''];
      lists.push(
        <tr key={-i} className={css.self_print_table}>
          <td>{i + 1}</td>
          <td colSpan={2}><p className={css.self_print_table_p}>{item[0]}</p></td>
          <td>{item[1] == "perfect" ? <Icon className={css.check_icon} type="check" /> : <span>&nbsp;</span>}</td>
          <td>{item[1] == "good" ? <Icon className={css.check_icon} type="check" /> : <span>&nbsp;</span>}</td>
          <td>{item[1] == "normal" ? <Icon className={css.check_icon} type="check" /> : <span>&nbsp;</span>}</td>
          <td>{item[1] == "bad" ? <Icon className={css.check_icon} type="check" /> : <span>&nbsp;</span>}</td>
        </tr>
      );
    }
    return lists;
  ...
  getData() {
    ...
    var url = `http://stiei-api.tallty.com/admin/results/${this.props.id}/feedback_form?random=${Math.random()}`
    SuperAgent.get(url)
      ...
      .end((err, res) => {
        if (res.ok) {
          ...
          this.setState({
            ...
            little_iterm: little_iterm,
            average_for_all_evaluation: average_for_all_evaluation,
            average_for_each_role: average_for_each_role,
            statistics_result: statistics_result,
            set_by_admin: set_by_admin,
            count_of_people: count_of_people,
            level_count_and_percentage: level_count_and_percentage,
            name: name,
            duty: duty,
            printLoading: false
          })
        ...
        <div id="print_table_one">
          ...
          <div className={css.print_table}>
            ...
            <div className={css.table_cell_one}>
              ...
              <Button className={css.print_btn_one} loading={this.state.printLoading} onClick={this.preview.bind(this, "print_table_one")} type="primary">打印表格</Button>
            </div>
