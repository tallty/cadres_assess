...
class Statistics extends Component {
  ...
  setEvaluateByAdministrator(values) {
    SuperAgent
      .patch(`http://stiei-api.tallty.com/admin/results/${this.state.logic_record.id}/set_final_result`)
      ...
      .end((err, res) => {
        if (!err || err === null) {
          ...
          console.log(data);
          this.setState({ data, modal_loading: false, visible: false });
        } else {
  ...
  setLevelByAdministrator(values) {
    SuperAgent
      .patch(`http://stiei-api.tallty.com/admin/results/${this.state.logic_record.id}/set_average_level`)
      .set('Accept', 'application/json')
      .set('Cache-control', 'no-cache')
      .set('X-Admin-Token', sessionStorage.admin_token)
      .set('X-Admin-Email', sessionStorage.admin_email)
  ...
  getData(year) {
    const url = `http://stiei-api.tallty.com/admin/results?activity_year=${year}`;
    SuperAgent
      .get(url)
      .set('Accept', 'application/json')
      .set('X-Admin-Token', sessionStorage.admin_token)
  ...
  render() {
    ...
    const url = `http://stiei-api.tallty.com/admin/output_result_index?activity_year=${this.props.location.query.year}`;
      ...
      <Admin>
        <div className={css.table_content}>
          <div className={css.table_title}>
            <span>{this.props.location.query.year}</span>考核统计结果列表
          </div>
